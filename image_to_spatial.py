...
def pixel_to_spatial_coordinates(pixel_x, pixel_y, image_path):
    ...
    # 计算每个像素对应的实际距离（米）
    # 假设图片覆盖20×20米的区域
    meters_per_pixel_x = 50.0 / img_width
    meters_per_pixel_y = 50.0 / img_height
...
def process_images_in_directory():
    ...
    # 这里需要替换为您实际的边界点提取逻辑
    #读取地块geojson文件
    geojson_path = "pinghexian.json"
    # 读取geojson文件 将要素存储为shapely对象
    #支持根据输入的要素判断是否与这些面要素相交
    polygons = []
    polygon_union = None
    spatial_index = None
    ...
    if os.path.exists(geojson_path):
        ...
        # 方法1：创建多边形并集，只需要进行一次相交判断
        try:
            polygon_union = unary_union(polygons)
            print("已创建多边形并集，将使用并集进行相交判断")
        except Exception as e:
            print(f"创建多边形并集失败：{e}，将使用空间索引")
            polygon_union = None
    ...
    for filename in os.listdir(pic_dir):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            #列号 行号 列号 行号
            box_xy = results_model[0].masks.xyn
            ...
            for i in range(len(box_cls)):
                ...
                boundary_pixels = [(512*float(i[0]), 512*float(i[1])) for i in local] #矩形四个顶点像素坐标

                # 绘制点到图像上
                # for px, py in boundary_pixels:
                #     cv2.circle(img, (int(px), int(py)), 2, (int(box_cls[i])*55, 0, 255-int(box_cls[i])*23), 4)
